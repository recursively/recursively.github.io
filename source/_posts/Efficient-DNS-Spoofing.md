...
This is not a tutorial of bettercap, acctually, I'm not familiar with many advanced features of it. Now let's get to know about the DNS spoofing in bettercap.
...
We use DNS spoofing module here and modify the configuration.
...
DNS requests use both TCP and UDP protocol, so we should make some changes to get it work.
...
So all the DNS requests traffic will be forworded to my attacking machine without changing the DNS server in every single host.
